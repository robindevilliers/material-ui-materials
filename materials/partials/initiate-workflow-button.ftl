<form action="${action}" style="${itemStyles}" method="post" enctype="application/x-www-form-urlencoded"
    ...
    <button id="${id}"
            onclick="${testMode?string('alert(&quot;Initiate workflow button clicked&quot;); event.preventDefault();','')}"
            style="${textualStyles}" class="initiate-workflow-button-widget btn ${classes}  ${textualClasses}"
